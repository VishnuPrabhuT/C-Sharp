...
namespace AADT
{
    public partial class AADTForm : Form
    {
        ...
        private void minionBackgroundWorker_DoWork(object sender, System.ComponentModel.DoWorkEventArgs e)
        {
            ...
            outputWriter.WriteLine("    County    " + "," + "    Station    " + "," + "    Functional_Class    " + "," + "    AADT    ");
            for (int i = 0; i < testData.Length; i++)
            {
                if (model_no[i] == 1)
                {
                    outputWriter.WriteLine(county[i] + "," + station[i] + "," + fClass[i] + "," + Math.Round(vol[i] / testResults1[i]));
                }
                if (model_no[i] == 2)
                {
                    outputWriter.WriteLine(county[i] + "," + station[i] + "," + fClass[i] + "," + Math.Round(vol[i] / testResults2[i]));
                }
                if (model_no[i] == 3)
                {
                    outputWriter.WriteLine(county[i] + "," + station[i] + "," + fClass[i] + "," + Math.Round(vol[i] / testResults3[i]));
                }
            }
            outputWriter.Flush();
            outputWriter.Close();
